...
//
// If you need multisign, you can use Transaction sign method add signature, more example in corechain-sdk/example/.
package service
...
func (x *XClient) initConn() error {
	...
	x.xconn = conn
	x.xc = pb.NewMXchainClient(conn)
	x.esc = protos.NewEventServiceClient(conn)
...
func (x *XClient) postTx(tx *pb.Transaction, bcname string) error {
	...
	if err != nil {
		fmt.Errorf("xuperclient post tx failed. error:%s", err)
	}
