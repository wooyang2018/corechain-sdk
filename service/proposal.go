...
import (
	...
	"github.com/wooyang2018/corechain-sdk/common"
...
func (p *Proposal) PreExecWithSelectUtxo() error {
	...
	if p.cfg.ComplianceCheck.IsNeedComplianceCheck {
		...
		if err != nil {
			return fmt.Errorf("EndorserCall PreExecWithFee failed. error:%s", err)
		}
		...
		if err != nil {
			return fmt.Errorf("PreExecWithSelectUTXO failed. error:%s", err)
		}
		...
		if p.request.opt.onlyFeeFromAccount {
			...
			if !ok {
				fmt.Errorf("invalid request fee. error:%s", common.ErrInvalidAmount)
			}
			...
			if err != nil {
				fmt.Errorf("SelectUTXO from contract account failed. error:%s", err)
			}
		}
	}
...
func (p *Proposal) complianceCheck(tx *pb.Transaction) (*pb.SignatureInfo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("EndorserCall ComplianceCheck failed. error:%s", err)
	}
...
func (p *Proposal) genComplianceCheckTx() (*pb.Transaction, error) {
	...
	err = common.SetSeed()
	if err != nil {
		return nil, fmt.Errorf("Set seed failed. error:%s", err)
	}
...
func (p *Proposal) signTx(tx *pb.Transaction) ([]byte, error) {
	...
	cryptoClient := common.GetCryptoClient()
	...
	if err != nil {
		return nil, fmt.Errorf("Make transaction ID failed. error:%s", err)
	}
